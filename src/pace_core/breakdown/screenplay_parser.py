...
and context rather than by column:
    ...
    parenthetical   (singing), (to the image of Ethan)
...
# (CONT'D), (V.O.), (O.S.), (to the image of Ethan) trailing a cue.
...
def _is_cue(line: str) -> bool:
    ...
    if not re.search(r"[A-Z一-鿿]", s):
        return False
    # Allow a name plus a parenthetical age note: "EMILY and RYAN(50's)"
    core = re.sub(r"\([^)]*\)", "", s)
