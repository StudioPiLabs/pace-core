...
On the over-the-shoulder of scene_02's "Dad swivels his chair around to face
Ethan", with geometry, proxy hair and seed held fixed, the only variable that
decided whether Ryan stayed turned toward his son was his description: given
...
KB = {
    "ryan": {"sex": "male", "trigger": "ryan_male",
...
def test_the_back_view_keeps_hair_build_and_clothing():
    out = _character_back_hint("ryan@adult_50", KB)
...
def test_the_back_view_drops_the_face():
    out = _character_back_hint("ryan@adult_50", KB)
    for face in ("weathered features", "determined expression", "ryan_male"):
        assert face not in out


def test_hair_is_cut_where_the_face_clause_starts():
    assert _hair_phrase(KB["ryan"]["anchor"]) == "short greying dark hair"
    assert _hair_phrase(KB["emily"]["generic_anchor"]) == \
...
def test_behind_sees_every_back():
    assert seen_from_behind(_shot("behind"), ["ryan", "emily"], {}) == {"ryan", "emily"}


def test_an_ots_sees_the_backs_of_everyone_but_the_focus():
    pf = {"type": "character", "ref": "ethan"}
    assert seen_from_behind(_shot("ots"), ["ryan", "ethan"], pf) == {"ryan"}
...
def test_an_ots_without_a_focus_names_nobody():
    ...
    assert seen_from_behind(_shot("ots"), ["ryan", "ethan"], {}) == set()


def test_positions_that_face_the_cast_see_no_backs():
    for pos in ("front", "three_quarter", "profile", None):
        assert seen_from_behind(_shot(pos), ["ryan", "ethan"],
                                {"type": "character", "ref": "ethan"}) == set()
