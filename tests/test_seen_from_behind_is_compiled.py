...
Theo", with geometry, proxy hair and seed held fixed, the only variable that
decided whether Omar stayed turned toward his son was his description: given
...
KB = {
    "omar": {"sex": "male", "trigger": "omar_male",
...
def test_the_back_view_keeps_hair_build_and_clothing():
    out = _character_back_hint("omar@adult_50", KB)
...
def test_the_back_view_drops_the_face():
    out = _character_back_hint("omar@adult_50", KB)
    for face in ("weathered features", "determined expression", "omar_male"):
        assert face not in out


def test_hair_is_cut_where_the_face_clause_starts():
    assert _hair_phrase(KB["omar"]["anchor"]) == "short greying dark hair"
    assert _hair_phrase(KB["nina"]["generic_anchor"]) == \
...
def test_behind_sees_every_back():
    assert seen_from_behind(_shot("behind"), ["omar", "nina"], {}) == {"omar", "nina"}


def test_an_ots_sees_the_backs_of_everyone_but_the_focus():
    pf = {"type": "character", "ref": "theo"}
    assert seen_from_behind(_shot("ots"), ["omar", "theo"], pf) == {"omar"}
...
def test_an_ots_without_a_focus_names_nobody():
    ...
    assert seen_from_behind(_shot("ots"), ["omar", "theo"], {}) == set()


def test_positions_that_face_the_cast_see_no_backs():
    for pos in ("front", "three_quarter", "profile", None):
        assert seen_from_behind(_shot(pos), ["omar", "theo"],
                                {"type": "character", "ref": "theo"}) == set()
