...
The screen-order case is not hypothetical. AutomaticDrive's 2026-09-02 build
of scene_01_shot_03_panel_0003 rendered the three-person cabin reversed --
ryan, declared leftmost at x=0.38, came out on the right -- and nothing in the
...
def test_a_correctly_staged_panel_passes(tmp_path):
    out = _build(tmp_path, {"ryan": (10, 50), "emily": (80, 120),
                            "ethan": (150, 190)})
    r = evaluate(_spec(tmp_path, [("ryan", 0.38), ("emily", 0.5),
                                  ("ethan", 0.62)]), out)
    assert r.ok
    assert r.failures == []
...
def test_reversed_cast_fails(tmp_path):
    ...
    out = _build(tmp_path, {"ryan": (150, 190), "emily": (80, 120),
                            "ethan": (10, 50)})
    r = evaluate(_spec(tmp_path, [("ryan", 0.38), ("emily", 0.5),
                                  ("ethan", 0.62)]), out)
    assert not r.ok
    ...
    assert "ryan declared left of emily" in order.detail
...
def test_a_body_that_did_not_render_fails_visibility(tmp_path):
    ...
    assert "no matte: ethan" in vis.detail
...
def test_a_body_staged_outside_the_frustum_fails_visibility(tmp_path):
    ...
    out = _build(tmp_path, {"ryan": (10, 50), "emily": (0, 0)})
    r = evaluate(_spec(tmp_path, [("ryan", 0.38), ("emily", 0.5)]), out)
    vis = _clause(r, "required_entity_visibility")
...
def test_a_subject_below_the_area_floor_fails(tmp_path):
    out = _build(tmp_path, {"ryan": (10, 50), "emily": (100, 101)})
    r = evaluate(_spec(tmp_path, [("ryan", 0.38), ("emily", 0.5)]), out)
    area = _clause(r, "projected_subject_area")
    assert area.ok is False
    assert area.value < MIN_SUBJECT_AREA and "emily" in area.detail
...
def test_required_can_be_narrowed_to_the_beats_own_facts(tmp_path):
    ...
    out = _build(tmp_path, {"ryan": (10, 50)})
    spec = _spec(tmp_path, [("ryan", 0.38), ("emily", 0.5)])
    assert evaluate(spec, out).ok is False
    assert evaluate(spec, out, required={"ryan"}).ok is True
...
def test_a_single_subject_declares_no_screen_order(tmp_path):
    out = _build(tmp_path, {"ryan": (10, 50)})
    r = evaluate(_spec(tmp_path, [("ryan", 0.38)]), out)
...
def test_two_subjects_on_one_sight_line_fail(tmp_path):
    ...
    out = _build(tmp_path, {"emily": (60, 120), "ethan": (75, 135)})
    r = evaluate(_spec(tmp_path, [("emily", 0.5), ("ethan", 0.62)]), out)
...
def test_a_head_behind_the_set_fails(tmp_path):
    """Scene 2 again, one level out. ryan's body was 51% visible and his head
    ...
    out = _build(tmp_path, {"ryan": (10, 50)})
    _head(tmp_path / (out.name + ".head_ryan.png"), 10, 50)
    _visible(tmp_path / (out.name + ".visible_ryan.png"), 10, 50, y0=45, y1=80)
    r = evaluate(_spec(tmp_path, [("ryan", 0.38)]), out)
    rp = _clause(r, "read_point_visible")
    assert rp.ok is False and "ryan 0%" in rp.detail
    assert not r.ok
...
def test_a_head_the_camera_can_see_passes(tmp_path):
    out = _build(tmp_path, {"ryan": (10, 50)})
    _head(tmp_path / (out.name + ".head_ryan.png"), 10, 50)
    _visible(tmp_path / (out.name + ".visible_ryan.png"), 10, 50)
    assert _clause(evaluate(_spec(tmp_path, [("ryan", 0.38)]), out),
...
def test_action_readability_still_says_it_cannot_measure_an_action(tmp_path):
    ...
    _head(tmp_path / (out.name + ".head_ryan.png"), 10, 50)
    _visible(tmp_path / (out.name + ".visible_ryan.png"), 10, 50)
    far = _clause(evaluate(_spec(tmp_path, [("ryan", 0.38)]), out),
