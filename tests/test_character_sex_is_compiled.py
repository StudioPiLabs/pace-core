...
the whole prompt: the inert trigger word `nina_female`, emitted by
...
KB = {
    "omar":  {"sex": "male",   "generic_anchor": "athletic build, weathered features"},
    "nina": {"sex": "female", "generic_anchor": "slender build, lively eyes"},
...
def test_a_male_character_is_named_a_man():
    assert "man," in _character_hint("omar@adult_50", KB)


def test_a_female_character_is_named_a_woman():
    out = _character_hint("nina@adult_50", KB)
...
def test_the_age_prefix_still_comes_first():
    ...
    out = _character_hint("omar@adult_50", KB)
